...
def dash_connect():
    if current_user.is_authenticated or app.config["ENV"] == "development":
        ...
        ans = {"session": request.sid, "name": current_user.username}
        emit(
            "login_successful",
            ans,
            broadcast=True,
...
def dash_update_sensors():
    ...
    emit("update_sensors", data, room=request.sid, namespace="/dashboard")
...
def get_sensor_data(data):
    ...
    time_deltas = {
        "1m": {"delta": timedelta(minutes=60), "format": "%H:%M:%S"},
        "5m": {"delta": timedelta(minutes=300), "format": "%H:%M:%S"},
        "1h": {"delta": timedelta(hours=50), "format": "%H:%M:%S"},
        "1d": {"delta": timedelta(days=50), "format": "%d.%m.%y"},
    }

    now = datetime.utcnow()
    ans = {"device_id": data["device_id"], "sensor_id": data["sensor_id"], "data": {}}
    for period in data["periods"]:
        try:
            data = data_handler.provider.load_values(
                sensor_id, period, now - time_deltas[period]["delta"]
            )
            ans["data"][period] = {"timestamps": [], "values": []}
            for res in data:
                ans["data"][period]["timestamps"].append(
                    res[0].strftime(time_deltas[period]["format"])
                )
                ans["data"][period]["values"].append(res[1])
        except KeyError:
            print("Time period not configured")

        emit("update_plot_data", ans, room=request.sid, namespace="/dashboard")
