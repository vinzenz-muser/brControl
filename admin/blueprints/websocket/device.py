...
def new_data(data):
    ...
    if device:
        for key, val in data["data"].items():
            current_sensor = (
                Sensor.query.filter(Sensor.id == key)
                .filter(Sensor.deviceId == device.id)
                .first()
            )
            print(data)
            if current_sensor:
                ...
                emit(
                    "new_data", socket_response, namespace="/dashboard", broadcast=True
                )
...
def updated_targets(data):
    ...
    if device:
        for current_data in data:
            sensor = device.sensors.filter(
                Sensor.id == current_data["sensor_id"]
            ).first()
