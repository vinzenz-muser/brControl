...
class Provider(ProviderInterface):
    ...
    def load_values(
        ...
        relevant_values = (
            self.Value.query.filter(self.Value.timespan == timespan)
            .filter(self.Value.time.between(start, end))
            .all()
        )
        return [
            (relevant_value.time, relevant_value.value)
    ...
    def insert_value(
        ...
        add_val = self.Value(
            value=value, sensorId=sensorId, time=time, timespan=timespan
        )
